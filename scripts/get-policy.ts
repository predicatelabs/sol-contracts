...
async function main() {
  ...
  if (args.length === 0) {
    ...
    console.error(
      "Example: ts-node scripts/get-policy.ts DNqiXmRMXgcGaqFJAegJT4EZd7e6b3S7mTpT3EsXMDdn"
    );
    process.exit(1);
  }
  ...
  console.log(
    "\n🔍 Looking up policy for program:",
    clientProgramId.toBase58()
  );
  ...
  try {
    ...
    console.log(
      "Set At:           ",
      new Date(policyAccount.setAt.toNumber() * 1000).toISOString()
    );
    console.log(
      "Updated At:       ",
      new Date(policyAccount.updatedAt.toNumber() * 1000).toISOString()
    );
    console.log("━".repeat(60));
    console.log("");
    ...
    console.log(
      "Policy Account:   ",
      `https://solscan.io/account/${policyPda.toBase58()}`
    );
    console.log(
      "Client Program:   ",
      `https://solscan.io/account/${clientProgramId.toBase58()}`
    );
    console.log("");
  } catch (error: any) {
    if (error.message?.includes("Account does not exist")) {
      ...
      console.log("");
      console.log(
        "This program does not have a policy set in the Predicate Registry."
      );
      console.log(
        "To set a policy, the program's upgrade authority must call set_policy_id."
      );
      console.log("");
    } else {
