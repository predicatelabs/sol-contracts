...
async function setCustomerPolicy(customerProgramId: string, policyId: string) {
  ...
  try {
    ...
    console.log(
      `   Set At: ${new Date(
        existingPolicy.setAt.toNumber() * 1000
      ).toISOString()}`
    );
    console.log();
  ...
  console.log(
    `   Set At: ${new Date(policy.setAt.toNumber() * 1000).toISOString()}`
  );
  console.log(
    `   Updated At: ${new Date(
...
if (args.length !== 2) {
  console.error(
    "Usage: npx ts-node scripts/set-customer-policy.ts <customer-program-id> <policy-id>"
  );
  console.error(
    "Example: npx ts-node scripts/set-customer-policy.ts C4DUV... customer-policy-v1"
  );
  process.exit(1);
}
