...
describe("Integration Tests", () => {
  ...
  describe("Complete Registry Workflow", () => {
    ...
    it("Should handle policy updates after attestor changes", async () => {
      ...
      // Deregister attestor
      const [attestorPda] = findAttestorPDA(attestor1.publicKey, context.program.programId);
      await context.program.methods
  ...
  describe("Cross-Operation Error Handling", () => {
    ...
    it("Should handle policy operations with non-existent attestors", async () => {
      ...
      await setPolicy(context.program, client1, Buffer.from("policy-without-attestors"), context.registry.registryPda);
      ...
      const updatedPolicyAccount = await context.program.account.policyAccount.fetch(policyPda);
  ...
  describe("State Consistency", () => {
    it("Should maintain consistent state across mixed operations", async () => {
      ...
      const operations = [
        ...
        async () => {
          await setPolicy(context.program, client1, Buffer.from("policy1"), context.registry.registryPda);
        },
        async () => {
          await registerAttestor(context.program, context.authority.keypair, attestor2.publicKey, context.registry.registryPda);
        },
        async () => {
          await setPolicy(context.program, client2, Buffer.from("policy2"), context.registry.registryPda);
        },
        async () => {
          const [policyPda] = findPolicyPDA(client1.publicKey, context.program.programId);
          await context.program.methods
      ...
      // Verify policies
      const [policy1Pda] = findPolicyPDA(client1.publicKey, context.program.programId);
      const [policy2Pda] = findPolicyPDA(client2.publicKey, context.program.programId);
    ...
    it("Should handle registry statistics correctly across all operations", async () => {
      ...
      const attestors = [attestor1, attestor2, Keypair.generate()];
      for (const attestor of attestors) {
        await registerAttestor(context.program, context.authority.keypair, attestor.publicKey, context.registry.registryPda);
      }
