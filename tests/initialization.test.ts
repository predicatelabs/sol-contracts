...
describe("Registry Initialization", () => {
  ...
  describe("Successful Initialization", () => {
    it("Should have registry initialized with correct initial state", async () => {
      ...
      expect(registryAccount.totalAttestors.toNumber()).to.be.greaterThanOrEqual(0);
      expect(registryAccount.totalPolicies.toNumber()).to.be.greaterThanOrEqual(0);
      ...
      const accountInfo = await context.provider.connection.getAccountInfo(context.registry.registryPda);
      expect(accountInfo).to.not.be.null;
      expect(accountInfo!.owner.toString()).to.equal(context.program.programId.toString());
  ...
  describe("Initialization Errors", () => {
    it("Should fail when trying to initialize an already initialized registry", async () => {
      ...
      try {
        ...
        expect.fail("Expected initialization to fail for already initialized registry");
      } catch (error: any) {
