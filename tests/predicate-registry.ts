...
describe("Predicate Registry", () => {
    ...
    it("Should fail to transfer authority with wrong current authority", async () => {
      try {
        await program.methods
          ...
          .accounts({
            registry: registryPda,
            authority: authority.publicKey, // Old authority, should fail
  ...
  describe("Edge Cases and Error Conditions", () => {
    ...
    it("Should handle re-registration of deregistered attestor", async () => {
      // Re-register the previously deregistered attestor1
      const [attestor1Pda] = PublicKey.findProgramAddressSync(
        [Buffer.from("attestor"), attestor1.publicKey.toBuffer()],
        program.programId
      );
    ...
    it("Should maintain correct registry statistics", async () => {
      ...
      // Should have 3 attestors now (attestor2 + newAttestor + re-registered attestor1)
      expect(registryAccount.totalAttestors.toNumber()).to.equal(3);
      
      // Should have 3 policies (client1, client2, maxClient)
      expect(registryAccount.totalPolicies.toNumber()).to.equal(0); // Note: policies don't increment registry counter in current implementation
    });
  });
});
